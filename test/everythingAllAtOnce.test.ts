...
describe('TOTAL => EXTREME RANDOM COMPLEX USAGE WITH OBSTACLES', () => {
    ...
    .it('orientation: %s, lat: %s, lng: %s, planetSize: %s, nbCommand: %s, nbObstacle: %s', 
    (orientation: Orientation, latStart: number, lngStart: number, planetSize: number, nbCommand: number, nbObstacle: number) => {
    
        const planet = new Planet(new Size(planetSize), new Size(planetSize), false);
        const planetWithObstacles = new  PlanetWithObstacles(new Size(planetSize), new Size(planetSize), generateObstacles(nbObstacle, planetSize, planet));
